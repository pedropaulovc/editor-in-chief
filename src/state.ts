...
export class State {
 ...
 `);
  ...
  putEvidence(items: Evidence[]): number {
    let changed = 0;
    this.db.transaction(() => {
      for (const e of items) {
        const old = this.db
          .query("SELECT revision FROM evidence WHERE id=?")
          .get(e.id) as { revision: string } | null;
        if (old?.revision === e.revision) continue;
        this.db
          .query(
            "INSERT INTO evidence VALUES(?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET revision=excluded.revision, observed_at=excluded.observed_at,data=excluded.data",
          )
          .run(e.id, e.revision, e.source, e.observedAt, JSON.stringify(e));
        changed++;
      }
    })();
    return changed;
  }
  evidence(limit = 500): Evidence[] {
    return (
      this.db
        .query("SELECT data FROM evidence ORDER BY observed_at DESC LIMIT ?")
        .all(limit) as { data: string }[]
    ).map((r) => JSON.parse(r.data));
  }
  candidates(): Candidate[] {
    return (
      this.db.query("SELECT data FROM candidates").all() as { data: string }[]
    ).map((r) => JSON.parse(r.data));
  }
  putCandidate(c: Candidate) {
    this.db
      .query(
  ...
  publications(): Publication[] {
    return (
      this.db.query("SELECT data FROM publications").all() as { data: string }[]
    ).map((r) => JSON.parse(r.data));
  }
  close() {
    this.db.close();
