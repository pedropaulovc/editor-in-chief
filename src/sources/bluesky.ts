...
const METHODS: Record<string, true> = {
  "com.atproto.identity.resolveHandle": true,
...
const PostSchema = z.object({
  uri: z
    .string()
    .regex(/^at:\/\/did:[^/]+\/app\.bsky\.feed\.post\/[A-Za-z0-9._~-]+$/),
  cid: z.string().min(1),
  author: ActorSchema,
  record: z.object({ text: z.string().max(30_000), createdAt: z.string() }),
  indexedAt: z.string().optional(),
  likeCount: z.number().nonnegative().optional(),
  repostCount: z.number().nonnegative().optional(),
  replyCount: z.number().nonnegative().optional(),
  quoteCount: z.number().nonnegative().optional(),
});
  ...
  feed: z
    .array(
      z.object({
        post: PostSchema,
        reason: z
          .object({ $type: z.string(), indexedAt: z.string().optional() })
          .passthrough()
          .optional(),
      }),
    )
...
const ProfileSchema = z.object({
  did: z.string(),
  followersCount: z.number().nonnegative().optional(),
  followsCount: z.number().nonnegative().optional(),
  postsCount: z.number().nonnegative().optional(),
});
...
function failure(error: unknown): string {
  return error instanceof SourceError
    ? error.message
    : "Bluesky operation failed; response body withheld";
}
async function request(
  ...
  if (!METHODS[method])
    throw new SourceError("Bluesky endpoint is not allowlisted");
  for (let attempt = 0; ; attempt++) {
    let response: Response;
    try {
      response = await fetch(`${HOST}/xrpc/${method}?${params}`, {
        redirect: "error",
        signal: AbortSignal.any([ctx.signal, AbortSignal.timeout(30_000)]),
      });
    } catch {
      throw new SourceError(
        `Bluesky ${method}: request failed or deadline exceeded`,
      );
    }
    if (attempt < 2 && (response.status === 429 || response.status >= 500)) {
      const retry = Number(response.headers.get("retry-after"));
      await response.body?.cancel();
      if (Number.isFinite(retry) && retry > 10)
        throw new SourceError(
          `Bluesky ${method}: HTTP ${response.status}; retry deferred`,
        );
    ...
    if (!response.ok) {
      await response.body?.cancel();
      throw new SourceError(`Bluesky ${method}: HTTP ${response.status}`);
    }
    if (Number(response.headers.get("content-length")) > 3_000_000) {
      await response.body?.cancel();
      throw new SourceError(`Bluesky ${method}: response bound exceeded`);
    }
    try {
      const text = await response.text();
      if (text.length > 3_000_000) throw new Error();
      return JSON.parse(text);
    } catch {
      throw new SourceError(`Bluesky ${method}: invalid or oversized response`);
    }
  }
}
async function resolve(ctx: Context): Promise<string> {
  const result = z
    .object({ did: ActorSchema.shape.did })
    .safeParse(
      await request(
        ctx,
        "com.atproto.identity.resolveHandle",
        new URLSearchParams({ handle: ctx.config.blueskyHandle }),
      ),
    );
  if (!result.success)
    throw new SourceError("Bluesky handle resolution schema invalid");
...
async function profile(ctx: Context, did: string) {
  ...
  if (!result.success || result.data.did !== did)
    throw new SourceError(
      "Bluesky profile identity mismatch or invalid schema",
    );
  return result.data;
}
async function feed(ctx: Context, did: string, cursor?: string) {
  ...
  const result = FeedSchema.safeParse(
    await request(ctx, "app.bsky.feed.getAuthorFeed", params),
  );
  if (!result.success)
    throw new SourceError("Bluesky author feed schema invalid");
  return result.data;
}
...
function evidenceFor(
  ctx: Context,
  post: Post,
  role: "author" | "reader-feedback",
  root?: string,
): Evidence {
  const sanitized = sanitize(post.record.text);
  const text = sanitized.slice(0, 5800);
  if (!Number.isFinite(Date.parse(post.record.createdAt)))
    throw new SourceError("Bluesky post timestamp invalid");
  return {
    id: `bluesky:${post.uri}`,
    source: "bluesky",
    sourceUrl: sourceUrl(post.uri),
    observedAt: ctx.now.toISOString(),
    occurredAt: post.record.createdAt,
    revision: post.cid,
    text:
      role === "author"
        ? text
        : `Reader feedback on a tracked engineering story; not Pedro's work or a verified claim.\n${text}`,
    provenance: {
      actorRole: role,
      authorDid: role === "author" ? post.author.did : undefined,
      root: root ?? null,
      verification: "public-source",
      textTruncated: sanitized.length > 5800,
      contextPolicy:
        "Quote embeds, reply roots, and third-party reposts are not attributed to Pedro. Unrelated personal/political posts must not become engineering assignments.",
    },
  };
}
function trackedUris(ctx: Context, did: string): string[] {
  ...
  for (const publication of ctx.state.publications()) {
    ...
    if (url.protocol !== "https:" || url.hostname !== "bsky.app" || !match)
      continue;
    let actor: string;
    try {
  ...
  for (const candidate of ctx.state.candidates()) {
    if (
      !["Selected", "Drafting", "Review", "Ready", "Published"].includes(
        candidate.status ?? "",
      )
    )
      continue;
    for (const id of candidate.sourceIds) {
      if (id.startsWith(`bluesky:at://${did}/app.bsky.feed.post/`))
        uris.add(id.slice("bluesky:".length));
    }
  }
  return [...uris].sort();
}
async function readerReplies(
  ctx: Context,
  root: string,
  did: string,
): Promise<{ evidence: Evidence[]; withheld: number; partial: boolean }> {
  const result = z
    .object({ thread: z.unknown() })
    .safeParse(
      await request(
        ctx,
        "app.bsky.feed.getPostThread",
        new URLSearchParams({ uri: root, depth: "2", parentHeight: "0" }),
      ),
    );
  ...
  while (nodes.length && visited < 60 && evidence.length < 20) {
    ...
    try {
      evidence.push(evidenceFor(ctx, post, "reader-feedback", root));
    } catch {
      withheld++;
    }
  }
...
export async function collectBluesky(ctx: Context): Promise<Collection> {
  ...
  try {
    ctx.state.set("source:bluesky:profile", {
      ...(await profile(ctx, did)),
      observedAt: ctx.now.toISOString(),
    });
  } catch (error) {
  ...
  state.pending ??= {
    from: new Date(
      state.checkpoint
        ? Date.parse(state.checkpoint) - 2 * DAY
    ...
    try {
      ...
      for (const entry of result.feed) {
        ...
        if (
          ...
          occurred < Date.parse(window.from) ||
          occurred > Date.parse(window.to)
        )
          continue;
        try {
          normalized.push(evidenceFor(ctx, entry.post, "author"));
        } catch {
          withheld++;
        }
      }
      ...
      const older =
        ...
        result.feed.every((entry) => {
          const activity =
            entry.reason?.indexedAt ??
            entry.post.indexedAt ??
            entry.post.record.createdAt;
          return (
            Number.isFinite(Date.parse(activity)) &&
            Date.parse(activity) < Date.parse(window.from)
          );
        });
      if (!result.cursor || older) {
        state.checkpoint = window.to;
        state.pending = undefined;
      } else {
        if (result.cursor === window.cursor)
          throw new SourceError("Bluesky feed cursor did not advance");
        window.cursor = result.cursor;
        window.pages++;
      }
      ctx.state.set(KEY, state);
    } catch (error) {
  ...
  const selected = [
    ...tracked.slice(offset),
    ...tracked.slice(0, offset),
  ].slice(0, 100);
  let hydrated = 0;
  for (
    let start = 0;
    start < selected.length && !ctx.signal.aborted;
    start += 25
  ) {
    try {
      ...
      const result = z
        ...
        .safeParse(await request(ctx, "app.bsky.feed.getPosts", params));
      if (!result.success)
        throw new SourceError("Bluesky tracked-post schema invalid");
      const normalized: Evidence[] = [];
      for (const post of result.data.posts) {
        if (!batch.includes(post.uri) || post.author.did !== did) continue;
        observation(ctx, post);
        try {
          normalized.push(evidenceFor(ctx, post, "author"));
        } catch {
          withheld++;
        }
      }
      const missing =
        ...
        result.data.posts.filter(
          (post) => batch.includes(post.uri) && post.author.did === did,
        ).length;
      if (missing)
        failures.push(
          `${missing} tracked Bluesky posts unavailable; not recorded as zero interactions`,
        );
      ctx.state.putEvidence(normalized);
      evidence.push(...normalized);
      hydrated += batch.length;
      state.hydrationOffset = (offset + hydrated) % Math.max(tracked.length, 1);
      ctx.state.set(KEY, state);
    } catch (error) {
      failures.push(failure(error));
      break;
  ...
  const roots = [
    ...tracked.slice(threadOffset),
    ...tracked.slice(0, threadOffset),
  ].slice(0, 4);
  ...
  for (let index = 0; index < roots.length && !ctx.signal.aborted; index++) {
    try {
      ...
      ctx.state.putEvidence(replies.evidence);
      evidence.push(...replies.evidence);
      withheld += replies.withheld;
      threadBound ||= replies.partial;
    } catch (error) {
      failures.push(failure(error));
    }
    state.threadOffset =
      (threadOffset + index + 1) % Math.max(tracked.length, 1);
    ctx.state.set(KEY, state);
  }
  if (withheld)
    failures.push(
      `Source-sanitization blocker: ${withheld} posts or replies withheld`,
    );
  const partial =
    Boolean(state.pending) ||
    failures.length > 0 ||
    ctx.signal.aborted ||
    tracked.length > selected.length;
  return {
    evidence,
    coverage: {
      source: "bluesky",
...
export async function doctorBluesky(ctx: Context): Promise<unknown> {
  try {
    ...
    const second = first.cursor
      ? await feed(ctx, did, first.cursor)
      : undefined;
    ...
    return {
      ...
      available: true,
      did,
      profileResolved: actor.did === did,
      firstPageCount: first.feed.length,
      pagination: second ? "verified" : "no next cursor available",
      secondPageCount: second?.feed.length ?? 0,
      parameters: {
        limit: 100,
        filter: "posts_with_replies",
        includePins: false,
      },
      metrics: "public counters only; no impressions or clicks",
    };
  } catch (error) {
    return { source: "bluesky", available: false, error: failure(error) };
  }
}
