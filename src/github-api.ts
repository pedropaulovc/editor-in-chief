export class ApiError extends Error {
  constructor(
    ...
    super(`GitHub ${endpoint.split("?")[0]}: HTTP ${status}`);
  }
}
...
export async function ghResponse<T = any>(
  endpoint: string,
  ...
  for (let attempt = 0; ; attempt++) {
    ...
    const split = out.search(/\r?\n\r?\n/),
      headerText = split < 0 ? "" : out.slice(0, split),
      body = split < 0 ? out : out.slice(split).replace(/^\r?\n\r?\n/, "");
    ...
    const status = Number(
      headerText.match(/^HTTP\/\S+ (\d+)/)?.[1] ??
        err.match(/HTTP (\d+)/)?.[1] ??
        0,
    );
    if (code === 0)
      return {
        data: body.trim() ? JSON.parse(body) : (undefined as T),
        headers,
      };
    const rateLimited =
      status === 429 ||
...
export async function gh<T = any>(
  ...
  return (await ghResponse<T>(endpoint, options)).data;
}
export async function graphql<T = any>(
  ...
  const result = await gh<any>("graphql", {
    method: "POST",
    body: { query, variables },
    signal,
  });
  if (result.errors?.length)
    throw new Error("GitHub GraphQL operation rejected");
  return result.data;
}
export async function pages<T = any>(
  endpoint: string,
  signal?: AbortSignal,
  maxPages = 100,
): Promise<T[]> {
  ...
  for (let page = 1; next && page <= maxPages; page++) {
    const response: ResponseData<T[]> = await ghResponse<T[]>(next, { signal });
    if (!Array.isArray(response.data))
      throw new Error("GitHub paginated endpoint returned non-array");
    all.push(...response.data);
    const url: string | undefined = response.headers.link
      ?.split(",")
    ...
    if (url) {
      ...
      if (parsed.origin !== "https://api.github.com")
        throw new Error("Unexpected GitHub pagination host");
      next = parsed.pathname.slice(1) + parsed.search;
    } else next = undefined;
  }
  if (next)
    throw new Error(`Pagination bound reached: ${endpoint.split("?")[0]}`);
  return all;
}
