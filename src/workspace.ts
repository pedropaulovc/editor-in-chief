...
const LOGBOOK =
  "https://github.com/pedropaulovc/harmonic-analyzer/blob/main/logbook/entries/TEMPLATE.md";
...
function repo(ctx: Context): string {
  return `repos/${ctx.config.repository}`;
}
function dry(ctx: Context): boolean {
  return ctx.mode === "dry-run";
}
function week(ctx: Context): string {
  return isoWeek(ctx.now, ctx.config.timezone);
}
function hasLabel(
  item: { labels: Array<{ name: string } | string> },
  label: string,
): boolean {
...
function marker(name: string): string {
  if (!/^[\w:.-]+$/.test(name))
    throw new Error("Invalid managed section identifier");
  return `<!-- eic:${name} -->`;
}
...
function managed(body: string | null | undefined, name: string): string | null {
  ...
  if (first < 0) {
    if (input.includes(end))
      throw new Error(`Malformed managed section ${name}`);
    return null;
  }
  const last = input.indexOf(end, first + start.length);
  if (
    last < 0 ||
    input.indexOf(start, first + start.length) >= 0 ||
    input.indexOf(end, last + end.length) >= 0
  )
    throw new Error(`Ambiguous managed section ${name}`);
...
function metadata<T>(body: string | null | undefined, name: string): T[] {
  ...
  while ((offset = (body ?? "").indexOf(prefix, offset)) >= 0) {
    ...
    try {
      result.push(JSON.parse(body!.slice(offset + prefix.length, end)) as T);
    } catch {
...
function proposal(
  ...
  const proposals = ctx.state.get<
    Array<{ key: string; kind: string; detail: unknown }>
  >("workspace:proposals", []);
  ctx.state.set("workspace:proposals", [
    ...proposals.filter((item) => item.key !== key),
    { key, kind, detail },
  ]);
}
function outbox(
  ...
  ctx.state.db
    .query(
      "INSERT INTO outbox(key,status,data,updated_at) VALUES(?,?,?,?) ON CONFLICT(key) DO UPDATE SET status=excluded.status,data=excluded.data,updated_at=excluded.updated_at",
    )
    .run(key, status, JSON.stringify({ kind, detail }), ctx.now.toISOString());
}
...
async function allIssues(ctx: Context): Promise<Issue[]> {
  return (
    await pages<Issue>(
      `${repo(ctx)}/issues?state=all&sort=created&direction=asc`,
      ctx.signal,
    )
  ).filter((issue) => !issue.pull_request);
}
async function issue(ctx: Context, number: number): Promise<Issue> {
  return gh<Issue>(`${repo(ctx)}/issues/${number}`, { signal: ctx.signal });
}
async function comments(ctx: Context, number: number): Promise<Comment[]> {
  return pages<Comment>(`${repo(ctx)}/issues/${number}/comments`, ctx.signal);
}
function unique<T>(items: T[], description: string): T | null {
  if (items.length > 1)
    throw new Error(
      `Ambiguous ${description}; refusing to choose between ${items.map((item) => (item as any).html_url ?? (item as any).url ?? (item as any).id).join(", ")}`,
    );
  return items[0] ?? null;
}
async function findIssue(ctx: Context, name: string): Promise<Issue | null> {
  return unique(
    (await allIssues(ctx)).filter((item) => managed(item.body, name) !== null),
    `issue marker ${name}`,
  );
}
async function ensureIssue(
  ...
  return deliver(
    ...
    `issue:${name}`,
    "create-issue",
    { name, title: text(title), labels, body },
    () => findIssue(ctx, name),
    () =>
      gh<Issue>(`${repo(ctx)}/issues`, {
        method: "POST",
        body: { title: text(title), body, labels },
        signal: ctx.signal,
      }),
  );
}
async function updateIssue(
  ...
  await deliver(
    ctx,
    `issue:${number}:${name}:${digest(desired)}`,
    "update-issue-section",
    { number, section: name, content: desired },
    async () =>
      managed((await issue(ctx, number)).body, name) === desired ? true : null,
    async () => {
      const fresh = await issue(ctx, number);
      const body = merge(fresh.body, name, desired);
      if (body !== (fresh.body ?? ""))
        await gh(`${repo(ctx)}/issues/${number}`, {
          method: "PATCH",
          body: { body },
          signal: ctx.signal,
        });
...
async function updateComment(
  ...
  const locate = async () =>
    unique(
      (await comments(ctx, number)).filter(
        (item) =>
          item.user.login === ctx.config.githubOwner &&
          managed(item.body, name) !== null,
      ),
      `comment marker ${name}`,
    );
  await deliver(
    ctx,
    `comment:${number}:${name}:${digest(desired)}`,
    "upsert-comment-section",
    { number, section: name, content: desired },
    async () => {
    ...
    async () => {
      const current = await locate();
      if (current) {
        const fresh = await gh<Comment>(
          `${repo(ctx)}/issues/comments/${current.id}`,
          { signal: ctx.signal },
        );
        await gh(`${repo(ctx)}/issues/comments/${current.id}`, {
          method: "PATCH",
          body: { body: merge(fresh.body, name, desired) },
          signal: ctx.signal,
        });
      } else
        await gh(`${repo(ctx)}/issues/${number}/comments`, {
          method: "POST",
          body: { body: block(name, desired) },
          signal: ctx.signal,
        });
      return true;
    },
...
async function repositoryId(ctx: Context): Promise<string> {
  const current = await gh<{ node_id: string; private: boolean }>(repo(ctx), {
    signal: ctx.signal,
  });
  if (current.private)
    throw new Error("The editorial repository must be public");
  return current.node_id;
}
async function discoverProject(ctx: Context): Promise<any | null> {
  const projects = await connections<any>(
    async (cursor) =>
      (
        await graphql<any>(
          "query($login:String!,$cursor:String){user(login:$login){projectsV2(first:100,after:$cursor){nodes{id title url public closed} pageInfo{hasNextPage endCursor}}}}",
          { login: ctx.config.githubOwner, cursor },
          ctx.signal,
        )
      ).user.projectsV2,
  );
  ...
  for (const project of projects.filter(
    (item) => item.title === "Editor-in-chief",
  )) {
    const linked = await connections<{ nameWithOwner: string }>(
      async (cursor) =>
        (
          await graphql<any>(
            "query($id:ID!,$cursor:String){node(id:$id){... on ProjectV2{repositories(first:100,after:$cursor){nodes{nameWithOwner} pageInfo{hasNextPage endCursor}}}}}",
    ...
    if (
      linked.some(
        (item) =>
          ...
          ctx.config.repository.toLowerCase(),
      )
    )
  ...
  return unique(
    matches,
    "Editor-in-chief projects with exact title and repository linkage",
  );
}
async function projectFields(ctx: Context, id: string): Promise<any[]> {
  return connections<any>(
    async (cursor) =>
      (
        await graphql<any>(
          "query($id:ID!,$cursor:String){node(id:$id){... on ProjectV2{fields(first:100,after:$cursor){nodes{... on ProjectV2Field{id name} ... on ProjectV2IterationField{id name} ... on ProjectV2SingleSelectField{id name options{id name color description}}} pageInfo{hasNextPage endCursor}}}}}",
          { id, cursor },
          ctx.signal,
        )
      ).node.fields,
...
async function getProject(
  ...
  if (!found && create) {
    ...
    found = await deliver(
      ...
      "project:create",
      "create-project",
      { owner, repository, title: "Editor-in-chief" },
      () => discoverProject(ctx),
      async () =>
        (
          await graphql<any>(
            "mutation($input:CreateProjectV2Input!){createProjectV2(input:$input){projectV2{id title url public closed}}}",
            {
              input: {
                ownerId: owner,
  ...
  if (!found) {
    if (dry(ctx)) {
      proposal(ctx, "project:create", "bootstrap-needed", {
        repository: ctx.config.repository,
      });
      return null;
    }
    throw new Error(
      "No linked Editor-in-chief project exists; run bootstrap --apply",
    );
  }
  if (found.closed)
    throw new Error(`Editorial project is closed: ${found.url}`);
  if (!found.public) {
    if (!create)
      throw new Error(`Editorial project is not public: ${found.url}`);
    await deliver(
      ctx,
      `project:${found.id}:public`,
      "make-project-public",
      { id: found.id },
      async () => ((await discoverProject(ctx))?.public ? true : null),
      async () => {
        await graphql(
          "mutation($input:UpdateProjectV2Input!){updateProjectV2(input:$input){projectV2{id}}}",
          { input: { projectId: found.id, public: true } },
          ctx.signal,
        );
        return true;
      },
  ...
  const loadStatus = async () =>
    unique(
      (await projectFields(ctx, found.id)).filter(
  ...
  if (status && !Array.isArray(status.options))
    throw new Error("Project Status field is not single-select");
  const missing = STATUSES.filter(
    (name) =>
      !status?.options.some((option: { name: string }) => option.name === name),
  );
  const extras = (status?.options ?? []).filter(
    (option: { name: string }) =>
      !STATUSES.some((name) => name === option.name),
  );
  if (missing.length || extras.length) {
    if (!create)
      throw new Error(
        "Project Status must contain exactly the seven editorial states; run bootstrap --apply",
      );
    if (extras.length) {
      const items = await boardItems(ctx, {
        id: found.id,
        title: found.title,
        url: found.url,
        public: found.public,
      ...
      if (
        items.some(
          (item) =>
            item.status && !STATUSES.some((name) => name === item.status),
        )
      )
        throw new Error(
          "Project has items using non-editorial statuses; map those items before removing unused defaults",
        );
    }
    status = await deliver(
      ctx,
      `project:${found.id}:status-options`,
      "configure-project-status",
      { id: found.id, statuses: STATUSES },
      async () => {
        const fresh = await loadStatus();
        return fresh &&
          fresh.options?.length === STATUSES.length &&
          STATUSES.every((name) =>
            fresh.options.some(
      ...
      async () => {
        const fresh = await loadStatus();
        const additions = STATUSES.filter(
          (name) => !fresh?.options.some((option: any) => option.name === name),
        ).map((name) => ({
          name,
          description:
            name === "Published"
              ? "Requires a registered original publication URL and date"
              : name === "Selected" || name === "Parked"
                ? "Pedro controls this state"
                : "",
          color:
            name === "Published" || name === "Ready"
              ? "GREEN"
              : name === "Parked"
                ? "GRAY"
                : "BLUE",
        ...
        if (!configured)
          throw new Error("Created Status field could not be recovered");
        return configured;
      },
    );
  }
  ...
  for (const name of STATUSES) {
    const match = unique(
      status.options.filter((option: any) => option.name === name),
      `Status option ${name}`,
    ) as any;
    if (!match) {
      if (dry(ctx)) continue;
      throw new Error(`Missing Status option ${name}`);
    }
...
async function boardItems(
  ...
  const nodes = await connections<any>(
    async (cursor) =>
      (
        await graphql<any>(
          'query($id:ID!,$cursor:String){node(id:$id){... on ProjectV2{items(first:100,after:$cursor){nodes{id content{... on Issue{id number repository{nameWithOwner}}} fieldValueByName(name:"Status"){... on ProjectV2ItemFieldSingleSelectValue{name optionId}}} pageInfo{hasNextPage endCursor}}}}}',
          { id: project.id, cursor },
          ctx.signal,
        )
      ).node.items,
...
async function boardItem(
  ctx: Context,
  project: Project,
  number: number,
): Promise<BoardItem | null> {
  return unique(
    (await boardItems(ctx, project)).filter((item) => item.issue === number),
    `project items for #${number}`,
  );
}
async function ensureItem(
  ...
  return deliver(
    ctx,
    `project:${project.id}:issue:${story.number}`,
    "add-project-item",
    { project: project.id, issue: story.number },
    () => boardItem(ctx, project, story.number),
    async () => {
      const added = (
        await graphql<any>(
          "mutation($input:AddProjectV2ItemByIdInput!){addProjectV2ItemById(input:$input){item{id}}}",
          { input: { projectId: project.id, contentId: story.node_id } },
          ctx.signal,
        )
      ).addProjectV2ItemById.item;
      return {
        id: added.id,
        contentId: story.node_id,
        issue: story.number,
        status: null,
      };
    },
...
async function transition(
  ...
  if (
    ...
    (expected === "Parked" ||
      expected === "Published" ||
      ctx.state.get(`workspace:manual-hold:${number}`, "") ===
        ctx.now.toISOString())
  )
    return false;
  if (
    ...
    !ctx.state
      .publications()
      .some((item) => item.storyIssue === number && item.kind === "original")
  )
    return false;
  if (!restore && (target === "Selected" || target === "Parked"))
    throw new Error("Selected and Parked are Pedro-only transitions");
  ...
  if (!current || current.status !== expected) {
    if (!dry(ctx))
      outbox(
        ctx,
        key,
        current?.status === target ? "delivered" : "cancelled",
        "project-status",
  ...
  if (dry(ctx)) {
    proposal(ctx, key, "project-status", { number, expected, target });
    return false;
  }
  ...
  const candidate = ctx.state
    .candidates()
    .find((item) => item.issueNumber === number);
...
function storyMeta(value: unknown): StoryMeta {
  ...
  if (
    ...
    !data.sourceIds.every((id) => typeof id === "string") ||
    !["harmonic-analyzer", "ai-engineering", "side-projects"].includes(
      data.pillar,
    ) ||
    !Number.isFinite(Date.parse(data.createdAt))
  )
    throw new Error("Invalid story recovery metadata");
  return data;
}
function candidateFromIssue(ctx: Context, current: Issue): Candidate {
  const saved = ctx.state
    .candidates()
    .find((candidate) => candidate.issueNumber === current.number);
  const metas = storyMetadata(current.body);
  if (metas.length > 1)
    throw new Error(`Ambiguous story metadata on #${current.number}`);
  const meta = metas[0] ? storyMeta(metas[0]) : null;
  if (saved && meta && saved.id !== meta.id)
    throw new Error(`Story identity changed on #${current.number}`);
  const result: Candidate = saved ?? {
    id: meta?.id ?? `issue-${current.number}`,
    topic: meta?.topic ?? current.title,
    sourceIds: meta?.sourceIds ?? [],
    pillar:
      meta?.pillar ??
      (hasLabel(current, "pillar:harmonic-analyzer")
        ? "harmonic-analyzer"
        : hasLabel(current, "pillar:ai-engineering")
          ? "ai-engineering"
          : "side-projects"),
    readerQuestions: meta?.brief?.questions ?? [],
    missingEvidence: meta?.brief?.missingEvidence ?? [],
    brief: meta?.brief ? BriefSchema.parse(meta.brief) : undefined,
    createdAt: meta?.createdAt ?? current.created_at ?? ctx.now.toISOString(),
    status: "Inbox",
  };
...
function storyMetadata(body: string | null | undefined): StoryMeta[] {
  const names = [
    ...(body ?? "").matchAll(/^<!-- eic:(story:[\w:.-]+) -->$/gm),
  ].map((match) => match[1]!);
  return names.flatMap((name) =>
    metadata<StoryMeta>(managed(body, name), "candidate-data"),
  );
}
function storyContent(
  ...
  if (!brief)
    throw new Error(`Candidate ${candidate.id} has no editorial brief`);
  ...
  if (sources.some((item) => !item))
    throw new Error(`Candidate ${candidate.id} is missing its cited evidence`);
  const citations = sources
    .map((source) => {
      const url = source!.sourceUrl;
      const publicLink =
        url && /^https:\/\/(?:github\.com|bsky\.app)\//.test(url)
          ? `[public source](${url.replaceAll(")", "%29")})`
          : "work notes; no public source link";
      const verified =
        source!.source === "hindsight" || source!.provenance.verified === false
          ? "**Reported in work notes; Pedro must verify**"
          : "Public source retrieved; cited material is not independent proof of every claim";
...
async function deskShell(ctx: Context): Promise<Issue | null> {
  ...
  return ensureIssue(
    ctx,
    `desk:${current}`,
    `Editorial desk — ${current}`,
    `This week's evidence-backed choices will appear here after the editorial desk runs. Nothing is assigned by this shell, and no missed week creates a debt.\n\n${CAPTURE_CHECKLIST}`,
    ["editorial-desk"],
  );
}
...
export async function bootstrap(ctx: Context): Promise<unknown> {
  ...
  for (const [name, description] of Object.entries(LABELS))
    await deliver(
      ...
      { name, description },
      async () =>
        (await pages<any>(`${repo(ctx)}/labels`, ctx.signal)).find(
      ...
      () =>
        gh(`${repo(ctx)}/labels`, {
          ...
          body: {
            name,
            description,
            color: name.startsWith("pillar:") ? "5319e7" : "0366d6",
          },
          signal: ctx.signal,
        }),
    );
  const project = await getProject(ctx, true);
  const inbox = await ensureIssue(
    ctx,
    "inbox",
    "Capture inbox",
    staticInbox(ctx),
...
function storePublication(ctx: Context, publication: Publication): void {
  ...
  const conflict = ctx.state
    .publications()
    .find(
      (item) =>
        item.storyIssue === publication.storyIssue &&
        item.channel === publication.channel &&
        item.url === publication.url &&
        publicationKey(item) !== key,
    );
  if (conflict)
    throw new Error(
      "Publication URL is already registered with different date or kind; refusing a conflicting record",
    );
  ctx.state.db
    .query(
      "INSERT INTO publications(key,data) VALUES(?,?) ON CONFLICT(key) DO NOTHING",
    )
    .run(key, JSON.stringify(publication));
}
function recoveredPublications(current: Issue): Publication[] {
  ...
  return metadata<unknown>(section, "publication-data").map((value) => {
    ...
    if (publication.storyIssue !== current.number)
      throw new Error(
        `Publication mirror refers to the wrong story on #${current.number}`,
      );
    return publication;
  });
}
async function mirrorPublications(
  ctx: Context,
  number: number,
  extra: Publication[] = [],
): Promise<void> {
  const fresh = await issue(ctx, number);
  const records = [
    ...recoveredPublications(fresh),
    ...ctx.state.publications().filter((item) => item.storyIssue === number),
    ...extra,
  ];
  const distinct = [
    ...new Map(records.map((item) => [publicationKey(item), item])).values(),
  ].sort(
    (a, b) =>
  ...
  ctx.state.db.transaction(() => {
    for (const record of distinct) {
      sanitize(record.url);
      if (!dry(ctx)) storePublication(ctx, record);
    }
  })();
...
export async function recordPublication(
  ...
  if (storyIssue.pull_request || !hasLabel(storyIssue, "editorial-story"))
    throw new Error(
      "Publication must reference an editorial-story issue in this repository",
    );
  if (!dry(ctx))
    ctx.state.db.transaction(() => {
      for (const existing of recoveredPublications(storyIssue))
        storePublication(ctx, existing);
      storePublication(ctx, publication);
    })();
...
function formPublication(current: Issue): Publication {
  ...
  return PublicationSchema.parse({
    storyIssue: Number(number),
    channel: fields["Channel"],
    url: fields["Publication URL"],
    publishedAt: fields["Published at"],
    kind: fields["Kind"],
  });
}
async function collectHumanInputs(
  ctx: Context,
  issues: Issue[],
): Promise<number> {
  let changed = 0;
  for (const current of issues) {
    ...
    if (
      capture &&
      current.user.login === ctx.config.githubOwner &&
      !current.body?.includes("<!-- eic:")
    )
      inputs.push({
        id: `github:capture:issue:${current.node_id}`,
        body: current.body ?? "",
        url: current.html_url,
        occurredAt: current.created_at ?? null,
        revision: current.updated_at ?? digest(current.body),
      });
    for (const comment of await comments(ctx, current.number))
      if (
        comment.user.login === ctx.config.githubOwner &&
        !comment.body.includes("<!-- eic:") &&
        comment.body.trim() !== "/eic review"
      )
        inputs.push({
          id: `github:capture:comment:${comment.id}`,
          body: comment.body,
          url:
            comment.html_url ??
            `${current.html_url}#issuecomment-${comment.id}`,
          occurredAt: comment.created_at ?? null,
          revision: comment.updated_at ?? digest(comment.body),
        });
    for (const input of inputs)
      if (input.body.trim())
        changed += ctx.state.putEvidence([
          {
            id: input.id,
            source: "github",
            sourceUrl: input.url,
            ...
            text: sanitize(input.body).slice(0, 12000),
            provenance: {
              authoredBy: ctx.config.githubOwner,
              capture: true,
              verified: false,
              attribution:
                "Pedro-authored rough notes; measurements are reported, not independently verified",
              storyIssue: story ? current.number : null,
            },
          },
        ]);
  }
...
export async function syncWorkspace(ctx: Context): Promise<unknown> {
  ...
  if (ctx.config.cadence === "paused")
    ctx.state.set("cadence:paused-weeks", [
      ...new Set([
        ...ctx.state.get<string[]>("cadence:paused-weeks", []),
        week(ctx),
      ]),
    ]);
  const issues = await allIssues(ctx);
  const inbox = unique(
    issues.filter((item) => managed(item.body, "inbox") !== null),
    "capture inbox",
  );
  if (inbox) ctx.state.set("workspace:inbox", inbox.number);
  for (const current of issues.filter((item) =>
    hasLabel(item, "editorial-desk"),
  )) {
    const metas = metadata<DeskMeta>(current.body, "desk-data");
    if (metas.length > 1)
      throw new Error(`Ambiguous desk metadata on #${current.number}`);
    const meta = metas[0];
    if (meta && /^\d{4}-W\d{2}$/.test(meta.week)) {
      ctx.state.set(`desk:${meta.week}:carry`, meta.carried);
      if (
        !dry(ctx) &&
        current.body?.includes(marker(`desk-complete:${meta.week}`))
      )
        ctx.state.set(`desk:${meta.week}:delivered`, current.html_url);
    }
  }
  ...
  for (const current of stories) {
    ...
    if (recoveredIds.has(candidate.id))
      throw new Error(
        `Duplicate issues for story ${candidate.id}; refusing to mutate either`,
      );
    recoveredIds.add(candidate.id);
    ctx.state.putCandidate(candidate);
    try {
      ctx.state.db.transaction(() => {
        for (const publication of recoveredPublications(current))
          storePublication(ctx, publication);
      })();
    } catch {
      await updateComment(
        ...
        "Conflicting or invalid mirrored publication records were withheld. Correct the URL, date/time or kind in the managed publication records; other stories can continue.",
      );
      continue;
    }
    if (
      ctx.state
        .publications()
        .some((item) => item.storyIssue === current.number)
    )
      await mirrorPublications(ctx, current.number);
    if (
      candidate.brief &&
      candidate.sourceIds.every((id) =>
        ctx.state.db.query("SELECT 1 FROM evidence WHERE id=?").get(id),
      )
    ) {
      const previousMeta = storyMetadata(current.body)[0];
      if (managed(current.body, `story:${candidate.id}`) !== null)
        await updateIssue(
          ctx,
          current.number,
          `story:${candidate.id}`,
          storyContent(ctx, candidate, previousMeta?.surfacedWeek ?? week(ctx)),
        );
    }
    if (!project) continue;
    const item = await ensureItem(ctx, project, current);
    if (!item) continue;
    const previous = ctx.state.get<string | null>(
      `workspace:verified-status:${current.number}`,
      null,
    );
    const original = ctx.state
      ...
      .some(
        (record) =>
          record.storyIssue === current.number && record.kind === "original",
      );
    if (item.status === "Published" && !original) {
      ...
      if (
        ...
        ctx.state.putCandidate({ ...candidate, status: restore });
      continue;
    }
    if (item.status && !STATUSES.includes(item.status as Status))
      throw new Error(
        `Unknown Status ${item.status} on story #${current.number}; choose one of the editorial statuses`,
      );
    if (
      item.status !== previous &&
      (item.status === "Selected" || item.status === "Parked")
    )
      ctx.state.set(
        `workspace:manual-hold:${current.number}`,
        ctx.now.toISOString(),
      );
  ...
  for (const form of issues.filter(
    (item) =>
      item.user.login === ctx.config.githubOwner &&
      hasLabel(item, "editorial-publication"),
  )) {
    const revision = digest(form.body);
    if (
      ctx.state.get(`publication-form:${form.number}:revision`, "") === revision
    )
      continue;
    let publication: Publication;
    try {
      publication = formPublication(form);
    } catch {
      await updateComment(
        ctx,
        form.number,
        "publication-form-error",
        "The publication form could not be registered. Supply a story issue number, one supported channel (blog/bluesky/linkedin/youtube/tiktok), a valid HTTPS URL, an explicit ISO date/time including timezone, and original or derivative. No submitted URL is fetched.",
      );
      continue;
    }
    try {
      ...
      await updateComment(
        ...
        form.number,
        "publication-form-error",
        "Publication registration could not complete. Check that the story is editorial, the URL/date/kind do not conflict with an existing record, and GitHub is reachable. No conflicting record was accepted.",
      );
      continue;
    }
    await updateIssue(
      ...
      form.number,
      "publication-receipt",
      `Registered ${publication.kind} / ${publication.channel} for story #${publication.storyIssue} at ${publication.publishedAt}. The durable publication record is in that story's managed publication section.`,
    );
    if (!dry(ctx))
      ctx.state.set(
        `publication-form:${form.number}:revision`,
        digest((await issue(ctx, form.number)).body),
      );
  }
  ...
  return {
    ...
    captures,
    publications: ctx.state.publications().length,
    active: ctx.state
      .candidates()
      .filter((candidate) => ACTIVE.has(candidate.status ?? ""))
      .map((candidate) => candidate.id),
  };
}

/** One weekly desk, at most three surfaced stories, never automatic selection or cadence growth. */
export async function applyDesk(
  ctx: Context,
  input: DeskResult,
): Promise<unknown> {
  await loadSafetySecrets(ctx.config.hindsightEnvFile);
  const desk = DeskResultSchema.parse(input),
    currentWeek = week(ctx);
  if (ctx.config.cadence === "paused") {
    ctx.state.set("cadence:paused-weeks", [
      ...new Set([
        ...ctx.state.get<string[]>("cadence:paused-weeks", []),
  ...
  if (existingDesk?.body?.includes(marker(`desk-complete:${currentWeek}`))) {
    if (!dry(ctx))
      ctx.state.set(`desk:${currentWeek}:delivered`, existingDesk.html_url);
  ...
  const active = candidates.filter(
    (candidate) =>
      ...
      !ctx.state
        ...
        .some(
          (publication) =>
            publication.storyIssue === candidate.issueNumber &&
            publication.kind === "original",
        ),
  );
  const cap = ctx.config.cadence === "twice-weekly" ? 2 : 1;
  if (
    new Set(desk.choices.map((choice) => choice.candidateId)).size !==
    desk.choices.length
  )
    throw new Error("Desk repeats a candidate");
  if (
    desk.recommendations.length > cap ||
    new Set(desk.recommendations).size !== desk.recommendations.length
  )
    throw new Error(
      "Desk recommendations exceed the operator-selected cadence",
    );
  for (const choice of desk.choices)
    if (!candidates.some((candidate) => candidate.id === choice.candidateId))
      throw new Error("Desk references an unknown candidate");
  for (const id of desk.recommendations)
    if (
      !desk.choices.some((choice) => choice.candidateId === id) &&
      !active.some((candidate) => candidate.id === id)
    )
      throw new Error(
        "Desk recommendation is not a supplied choice or active story",
      );
  const previousWeek = isoWeek(
    new Date(ctx.now.getTime() - 7 * 86400000),
    ctx.config.timezone,
  );
  const priorCarries = ctx.state.get<Record<string, number>>(
    `desk:${previousWeek}:carry`,
    {},
  );
  const carried = Object.fromEntries(
    active.map((candidate) => [
      candidate.id,
      (priorCarries[candidate.id] ?? 0) + 1,
    ]),
  );
  const capacity = active.some((candidate) => carried[candidate.id]! >= 2)
    ? 0
    : Math.max(0, cap - active.length);
  if (
    active.length &&
    desk.choices.filter(
      (choice) =>
        !active.some((candidate) => candidate.id === choice.candidateId),
    ).length > capacity
  )
    throw new Error("Desk choices exceed remaining original capacity");
  const surfaced = (await allIssues(ctx)).filter((item) =>
    storyMetadata(item.body).some((meta) => meta.surfacedWeek === currentWeek),
  );
  ...
  for (const choice of desk.choices) {
    ...
    const current =
      ...
      (await ensureIssue(
        ctx,
        `story:${candidate.id}`,
        `Story: ${text(choice.brief.topic)}`,
        storyContent(ctx, updated, currentWeek),
        ["editorial-story", `pillar:${candidate.pillar}`],
      ));
    if (!remote) created++;
    if (current) {
      ...
      await updateIssue(
        ...
        storyContent(
          ctx,
          updated,
          storyMetadata(current.body)[0]?.surfacedWeek ?? currentWeek,
        ),
      );
      if (project) {
        const item = await ensureItem(ctx, project, current);
  ...
  const recommended = [
    ...active.slice(0, cap).map((candidate) => candidate.id),
    ...desk.recommendations.filter(
      (id) =>
        choices.some((candidate) => candidate.id === id) &&
        !active.some((candidate) => candidate.id === id),
    ),
  ].slice(0, cap);
  const summary = deskMetrics(ctx);
  const prerequisites = (await allIssues(ctx))
    .filter((item) => hasLabel(item, "editorial-prerequisite"))
    .map((item) => `- [${text(item.title)}](${item.html_url})`)
    .join("\n");
  const meta: DeskMeta = {
    week: currentWeek,
    carried,
    choices: choices.map((candidate) => candidate.id),
    recommendations: recommended,
  };
  const carrySection = active.length
  ...
  const experiment =
    desk.experiment && !active.some((candidate) => carried[candidate.id]! >= 2)
      ...
      : "";
  const content = [
    `<!-- eic:desk-data:${json(meta)} -->`,
    marker(`desk-complete:${currentWeek}`),
    ...
    experiment,
    `### Publishing prerequisites\n${prerequisites}\n- [Harmonic Analyzer landing/email capture #472](${SIGNUP})`,
    CAPTURE_CHECKLIST,
  ]
    .filter(Boolean)
    .join("\n\n");
  ...
  if (current)
    await updateIssue(ctx, current.number, `desk:${currentWeek}`, content);
  ...
  if (!dry(ctx) && current) {
    ...
    for (const candidate of active)
      ctx.state.putCandidate({
        ...candidate,
        carriedWeeks: carried[candidate.id],
      });
  }
  return {
    status: dry(ctx) ? "proposed" : "delivered",
    url: current?.html_url ?? null,
    choices: choices.map((candidate) => candidate.id),
    recommended,
    carried,
    ...(dry(ctx)
      ? { briefs: choices.map((candidate) => candidate.brief), content }
      : {}),
  };
}
...
async function eligible(
  ...
  const pr = await gh<Pull>(`${repo(ctx)}/pulls/${number}`, {
    signal: ctx.signal,
  });
  if (
    pr.state !== "open" ||
    pr.user.login !== ctx.config.githubOwner ||
    !hasLabel(pr, "editorial-draft") ||
    pr.base.repo.full_name.toLowerCase() !== ctx.config.repository.toLowerCase()
  )
    return null;
  ...
  const changed = await pages<ChangedFile>(
    `${repo(ctx)}/pulls/${number}/files`,
    ctx.signal,
  );
  if (pr.changed_files !== undefined && changed.length !== pr.changed_files)
    throw new Error("PR file listing is incomplete; refusing partial review");
  ...
  if (
    ...
    changed.some(
      (file) =>
        ...
        !/\.(?:md|markdown|txt|vtt|srt|png|jpe?g|webp|gif|svg|mp4|mov|m4v|webm|mp3|wav|ogg|pdf)$/i.test(
          file.filename,
        ),
    )
  )
...
function reviewMarkers(body: string): string[] {
  return [
    ...body.matchAll(/<!-- eic:(review:\d+:[a-f0-9]+:[a-f0-9]+(?::\d+)?) -->/g),
  ].map((match) => match[1]!);
}
async function reviewAction(
  ...
  const [reviews, discussion] = await Promise.all([
    pages<Comment>(`${repo(ctx)}/pulls/${entry.pr.number}/reviews`, ctx.signal),
    comments(ctx, entry.pr.number),
  ]);
  const delivered = reviews
    .filter((review) => review.user.login === ctx.config.githubOwner)
    .flatMap((review) => reviewMarkers(review.body));
  if (!dry(ctx))
    for (const action of delivered)
      ctx.state.db
        .query(
          "UPDATE outbox SET status='delivered',updated_at=? WHERE key=? AND status IN ('pending','attempted')",
        )
        .run(ctx.now.toISOString(), action);
  const prefix = `review:${entry.pr.number}:${entry.pr.head.sha}:${policy}`;
  const requests = discussion
    .filter(
      (comment) =>
        comment.user.login === ctx.config.githubOwner &&
        comment.body.trim() === "/eic review",
  ...
  const requested = requests.find(
    (comment) => !delivered.some((action) => action.endsWith(`:${comment.id}`)),
  );
  if (requested)
    return {
      marker: `${prefix}:${requested.id}`,
  ...
  if (
    delivered.some(
      (action) => action === prefix || action.startsWith(prefix + ":"),
    )
  )
    return null;
...
export async function eligibleReviews(ctx: Context): Promise<number[]> {
  const policy = await policyHash(),
    result: number[] = [];
  for (const pr of await pages<Pull>(
    `${repo(ctx)}/pulls?state=open`,
    ctx.signal,
  )) {
    if (
      pr.user.login !== ctx.config.githubOwner ||
    ...
    if (entry && (await reviewAction(ctx, entry, policy)))
      result.push(pr.number);
  }
...
async function fetchDraft(
  ctx: Context,
  entry: Eligible,
): Promise<{ files: DraftFile[]; unseenMedia: string[] } | null> {
  ...
  }>(`${repo(ctx)}/git/trees/${entry.pr.head.sha}?recursive=1`, {
    signal: ctx.signal,
  });
  if (tree.truncated)
    throw new Error("Head-SHA tree is truncated; refusing partial review");
  const entries = tree.tree.filter(
    (item) => item.path.startsWith(prefix) && item.type !== "tree",
  );
  if (entries.length > 100)
    throw new Error("Draft exceeds the bounded 100-file review envelope");
  const files: DraftFile[] = [],
    unseenMedia: string[] = [];
  let bytes = 0;
  for (const item of entries) {
    if (item.mode === "120000" || item.type !== "blob")
      throw new Error("Draft symlinks and submodules are not reviewable");
    if (!/\.(?:md|markdown|txt|vtt|srt)$/i.test(item.path)) {
      if (
        /\.(?:png|jpe?g|webp|gif|svg|mp4|mov|m4v|webm|mp3|wav|ogg|pdf)$/i.test(
          item.path,
        )
      ) {
        unseenMedia.push(item.path);
        continue;
      }
      return null;
    }
    if ((item.size ?? 0) > 200000 || bytes + (item.size ?? 0) > 1000000)
      throw new Error("Draft exceeds the bounded text review envelope");
    const blob = await gh<{
      encoding: string;
      content: string;
      size: number;
      sha: string;
    }>(`${repo(ctx)}/git/blobs/${item.sha}`, { signal: ctx.signal });
    if (
      blob.encoding !== "base64" ||
      blob.sha !== item.sha ||
      blob.size > 200000
    )
      throw new Error("Unexpected draft blob encoding or identity");
    ...
    files.push({
      path: item.path,
      content,
      patch: changed.find((file) => file.filename === item.path)?.patch,
    });
  }
  return files.length ? { files, unseenMedia } : null;
}
async function threadStates(
  ctx: Context,
  number: number,
): Promise<Record<number, { resolved: boolean; outdated: boolean }>> {
  const [owner, name] = ctx.config.repository.split("/");
  const threads = await connections<any>(
    async (cursor) =>
      (
        await graphql<any>(
          "query($owner:String!,$name:String!,$number:Int!,$cursor:String){repository(owner:$owner,name:$name){pullRequest(number:$number){reviewThreads(first:100,after:$cursor){nodes{id isResolved isOutdated comments(first:100){nodes{databaseId} pageInfo{hasNextPage endCursor}}} pageInfo{hasNextPage endCursor}}}}}",
  ...
  for (const thread of threads) {
    const ids = thread.comments.pageInfo.hasNextPage
      ? await connections<any>(
          async (cursor) =>
            (
              await graphql<any>(
    ...
    for (const comment of ids)
      result[comment.databaseId] = {
...
function parsedReview(review: Comment): ReviewResult | null {
  ...
  if (entries.length !== 1)
    throw new Error("Ambiguous persisted review result");
...
async function head(ctx: Context, number: number): Promise<string> {
  return (
    await gh<Pull>(`${repo(ctx)}/pulls/${number}`, { signal: ctx.signal })
  ).head.sha;
}
async function discardHead(
  ctx: Context,
  number: number,
  actual: string,
): Promise<unknown> {
  ctx.state.set(`review:queued:${number}`, actual);
  return { status: "head-changed", headSha: actual, queued: true };
}
...
export async function reviewPullRequest(
  ...
  const policy = await policyHash(),
    action = await reviewAction(ctx, entry, policy);
  if (!action)
    return { status: "already-reviewed", headSha: entry.pr.head.sha };
  const project = await getProject(ctx),
    planned = project
      ? await boardItem(ctx, project, entry.issue.number)
      : null;
  const files = await fetchDraft(ctx, entry);
  if (!files) return { status: "ineligible-files" };
  const [inline, resolution] = await Promise.all([
    pages<Comment>(`${repo(ctx)}/pulls/${number}/comments`, ctx.signal),
    threadStates(ctx, number),
  ]);
  const priorFindings = action.reviews
    .filter((review) => review.user.login === ctx.config.githubOwner)
    .flatMap((review) => {
      const prior = parsedReview(review);
      return (prior?.findings ?? []).map((finding) => ({
        ...finding,
        reviewId: review.id,
        headSha: prior!.headSha,
        submittedAt: review.submitted_at,
        resolution:
          "Consult Pedro responses and current text; an old-head finding is not automatically unresolved.",
      }));
    });
  const responses = [
    ...action.comments
    ...
    ...inline
      .filter((comment) => comment.user.login === ctx.config.githubOwner)
      .map((comment) => ({
    ...
    ...action.reviews
      .filter(
        (review) =>
          review.user.login === ctx.config.githubOwner &&
          !review.body.includes("<!-- eic:"),
      )
      .map((review) => ({
        type: "human-review",
        id: review.id,
        text: sanitize(review.body),
      })),
    {
      type: "media-coverage",
      unseenMedia: files.unseenMedia,
      limitation:
        "Only fetched UTF-8 text is reviewed. Binary media and linked external media have not been inspected; essential unseen media cannot be treated as verified.",
    },
  ];
  const pending = ctx.state.db
    .query("SELECT key,status,data,updated_at FROM outbox WHERE key=?")
    .get(action.marker) as Outbox | null;
  ...
  if (pending && ["pending", "attempted"].includes(pending.status)) {
    ...
    if (result.headSha !== entry.pr.head.sha)
      return discardHead(ctx, number, entry.pr.head.sha);
  } else {
    const lookup = ctx.state.db.query("SELECT data FROM evidence WHERE id=?");
    const evidence = entry.story.sourceIds
      .slice(-12)
      .map((id) => {
        const row = lookup.get(id) as { data: string } | null;
        return row ? (JSON.parse(row.data) as Evidence) : null;
    ...
    const response = await editor(ctx, {
      kind: "review",
    ...
    if (response.result.kind !== "review")
      throw new Error("Editor returned the wrong result kind");
    result = ReviewResultSchema.parse(response.result);
  }
  if (result.headSha !== entry.pr.head.sha)
    throw new Error("Editor returned a review for the wrong head");
  ...
  const sent = await deliver(
    ...
    action.marker,
    "submit-review",
    { number, headSha: result.headSha, result, body },
    async () =>
      unique(
        (
          await pages<Comment>(
            `${repo(ctx)}/pulls/${number}/reviews`,
            ctx.signal,
          )
        ).filter(
          (review) =>
            review.user.login === ctx.config.githubOwner &&
            review.body.includes(marker(action.marker)),
        ),
        `review ${action.marker}`,
      ),
    async () => {
      ...
      return gh<Comment>(`${repo(ctx)}/pulls/${number}/reviews`, {
        method: "POST",
        body: { event: "COMMENT", commit_id: result.headSha, body },
        signal: ctx.signal,
      });
    },
  );
  ...
  if (sent && project && planned)
    await transition(
      ctx,
      project,
      entry.issue.number,
      planned.status,
      result.verdict === "ready" ? "Ready" : "Review",
    );
  if (sent && !dry(ctx)) {
    ctx.state.delete(`review:queued:${number}`);
    ctx.state.set(`review:last:${number}`, {
  ...
  if (sent && !dry(ctx))
    ctx.state.set(
      "pending-reviews",
      ctx.state
        .get<number[]>("pending-reviews", [])
        .filter((pr) => pr !== number),
    );
  return {
    status: dry(ctx) ? "proposed" : "reviewed",
    headSha: result.headSha,
    verdict: result.verdict,
    storyIssue: entry.issue.number,
    reviewId: sent?.id ?? null,
    ...(dry(ctx) ? { review: result, body } : {}),
  };
}
async function syncDraftStates(ctx: Context, project: Project): Promise<void> {
  ...
  const states = new Map<
    number,
    Array<{ pr: number; sha: string; status: Status; reviewId: number | null }>
  >();
  for (const pull of await pages<Pull>(
    `${repo(ctx)}/pulls?state=open`,
    ctx.signal,
  )) {
    if (
      pull.user.login !== ctx.config.githubOwner ||
      !hasLabel(pull, "editorial-draft")
    )
      continue;
    ...
    const reviews = (
      await pages<Comment>(
        `${repo(ctx)}/pulls/${pull.number}/reviews`,
        ctx.signal,
      )
    )
      .filter(
        (review) =>
          review.user.login === ctx.config.githubOwner &&
          reviewMarkers(review.body).some(
            (action) => action === prefix || action.startsWith(prefix + ":"),
          ),
      )
      .sort((a, b) => b.id - a.id);
    ...
    const status: Status =
      latest?.headSha === entry.pr.head.sha
        ? latest.verdict === "ready"
          ? "Ready"
          : "Review"
        : "Drafting";
    states.set(entry.issue.number, [
      ...(states.get(entry.issue.number) ?? []),
      {
        pr: pull.number,
        sha: entry.pr.head.sha,
        status,
  ...
  for (const [number, pulls] of states) {
    const eventKey = `workspace:draft-event:${number}`,
      event = digest(pulls);
    ...
    if (
      ...
      ctx.state
        ...
        .some(
          (publication) =>
            publication.storyIssue === number &&
            publication.kind === "original",
        )
    )
      continue;
    const target = pulls.some((pull) => pull.status === "Drafting")
      ? "Drafting"
      : pulls.some((pull) => pull.status === "Review")
        ? "Review"
        : "Ready";
    let unchanged = true;
    for (const pull of pulls)
      if ((await head(ctx, pull.pr)) !== pull.sha) {
        unchanged = false;
        break;
      }
    if (unchanged) {
...
export async function reportBlockers(
  ctx: Context,
  coverage: Coverage[],
  ...
  const failures = [
    ...coverage.flatMap((item) =>
      (item.status === "failed" ? [item.detail] : (item.blockers ?? [])).map(
        (detail) => ({
    ...
    ...[...new Set(operationErrors)].map((detail) => ({
      source: "editorial-operation",
      status: "failed",
      detail: sanitize(detail),
    })),
  ].sort(
    (a, b) =>
      a.source.localeCompare(b.source) || a.detail.localeCompare(b.detail),
  );
  const prior = ctx.state.get<{
    fingerprint: string;
    failures: unknown[];
  } | null>("workspace:blockers", null);
  const pending = ctx.state.get<{
    fingerprint: string;
    failures: unknown[];
  } | null>("workspace:blockers-pending", null);
  const fingerprint = digest(failures);
  // Save the alert before the network: an unavailable GitHub cannot erase it.
  if (failures.length || !pending?.failures.length)
    ctx.state.set("workspace:blockers-pending", { fingerprint, failures });
  const inbox = await findIssue(ctx, "inbox");
  if (!inbox) {
    if (failures.length && !dry(ctx))
      throw new Error(
        "Operational blocker retained locally; capture inbox unavailable until bootstrap",
      );
    if (failures.length)
      proposal(ctx, "operational-blockers", "blocker-comment", { failures });
    return {
      status: failures.length ? "pending-bootstrap" : "none",
      proposed: dry(ctx),
    };
  }
  const existing = unique(
    (await comments(ctx, inbox.number)).filter(
      (comment) =>
        comment.user.login === ctx.config.githubOwner &&
        managed(comment.body, "operational-blockers") !== null,
    ),
    "operational blocker comment",
  );
  if (
    !failures.length &&
    ...
    ctx.state.delete("workspace:blockers-pending");
    return { status: "none" };
  }
  const content = failures.length
    ? `### Operational blocker\n${failures.map((item) => `- **${item.source} / ${item.status}:** ${text(item.detail)}`).join("\n")}\n\nCoverage is incomplete; no claim depending on an unavailable fetch is newly verified. Safe work from successful sources continues. This one comment is updated on change, not reposted each scan.`
    : "### Operational blocker — resolved\nThe previously reported source/operation blockers have recovered. Normal collection can continue; this is not a publication reminder.";
  await updateComment(ctx, inbox.number, "operational-blockers", content);
  if (!dry(ctx)) {
    ctx.state.set("workspace:blockers", { fingerprint, failures });
    ctx.state.delete("workspace:blockers-pending");
  }
  return {
    status: failures.length ? "blocked" : "resolved",
